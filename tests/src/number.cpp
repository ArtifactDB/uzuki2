...
TEST(NumberTest, SimpleLoading) {
    auto path = "TEST-float.h5";

    // Simple stuff works correctly.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        auto vhandle = vector_opener(handle, "blub", "float");
    ...
    }
    {
        ...
        EXPECT_EQ(parsed->type(), uzuki2::FLOAT);
        auto bptr = static_cast<const DefaultDoubleVector*>(parsed.get());
...
TEST(NumberTest, CheckError) {
    ...
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        auto ghandle = vector_opener(handle, "foo", "float");
